...
> = {
  ...
  insurance: {
    ...
    en: `Coverage depends on your plan and the service. Call the billing office at ${site.phone} and we'll verify your benefits before your first appointment — there's no obligation.`,
  ...
  diagnosis: {
    ...
    en: `Jennifer Ramirez provides autism diagnostic evaluations using gold-standard protocols. It's about a 14-hour process across sessions, and the rate is quoted individually. Call ${site.phone} to get started.`,
  ...
  location: {
    ...
    en: `We're at ${fullAddress()}. There's parking directly outside and the entrance is on the ground floor. If it helps, there's a step-by-step walkthrough of a first visit on the site.`,
...
export function fallbackReply(message: string, locale: Locale): string {
  if (CRISIS.test(message)) {
    return locale === "es"
      ? `Lamento mucho que estés pasando por esto. Si tú o alguien más está en peligro inmediato, por favor llama al 911. Puedes hablar con alguien ahora mismo, gratis y en confianza, llamando o enviando un mensaje al 988 (Línea de Prevención del Suicidio y Crisis). Aquí en la clínica también podemos ayudarte a encontrar apoyo continuo: ${site.phone}.`
